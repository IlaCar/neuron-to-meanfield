"""
regime_of_validity_FS.py
------------------------
Manuscript 'regime of validity' figure for the FS mean field.

Three panels over the (exc, inh) input grid:
  1. closed-loop  MF / NN          (full MF vs network)
  2. open-loop    TF@op-point / NN (TF accuracy alone, loop removed)
...
# ============================ configure =============================
# FS is non-adaptive: only the baked-in dataset is meaningful (w_ad = 0).
TF_FILE = '10_best_params_TF_FS.json'
...
FS_json = '../../neuron_models/AdEx/FS.json'
grid    = np.arange(0, 31, 2)
# ====================================================================

net = get_network_config(json_file_name=cfg)
params = adding_K_params(get_params_model_SI('FS', FS_json), net)
...
for j, (ax, (M, title, cmap, kw)) in enumerate(zip(axes, panels)):
    ...
    ax.plot([0, 30], [0, 30], color='0.4', ls='--', lw=1)
    if j < 2:
