...
Qe, Qi = get_syn_info(json_file_name = os.path.join(data_folder, 'FS.json'))
### information to save ###
in_inh = []
in_exc = []
...
for i, ext_input_inh in enumerate(external_input_inh_range):
    for j, ext_input_exc in enumerate(external_input_exc_range):
        ...
        S_ed_background_bg = b2.Synapses(P_ed_bg, G_FS, on_pre='GsynE_post+=Qe')
        S_ed_background_bg.connect(p = conn_prob)
        
        # Connecting the external input to the neuron models
        S_ed_inh = b2.Synapses(P_ed_inh, G_FS, on_pre='GsynI_post+=Qi')
        S_ed_inh.connect(p = conn_prob)           
        S_ed_exc = b2.Synapses(P_ed_exc, G_FS, on_pre='GsynE_post+=Qe')
