...
simualtion_folder = os.path.join('TF_RS_w_v1')
...
# mean adaptation <w> per grid point (Amps, SI) — for the explicit-adaptation TF fit
W_avg = np.zeros((len(external_input_inh_range), len(external_input_exc_range)))
...
for i, ext_input_inh in enumerate(external_input_inh_range):
    for j, ext_input_exc in enumerate(external_input_exc_range):
        ...
        # Record the AdEx adaptation variable 'w' for every grid point so we can
        # characterise F(nu_e, nu_i, W). Recorded at 1 ms (enough for a window mean).

        mon_w_RS = b2.StateMonitor(G_RS, 'w', record=True, dt=1 * b2.ms)
        ...
        # Mean adaptation <w> over the SAME window used for the firing rate,
        # i.e. [p_start + delay, p_end]. Because the window tracks `delay`, this
        # is also correct for the no-delay variant (delay = 0). Averaged over
        # neurons and time; stored in SI Amps to match the w_ad convention used
        # in the TF fit and mean-field integration.
        w_t = mon_w_RS.t / b2.second
        w_win = (w_t >= (p_start + delay) / b2.second) & (w_t <= p_end / b2.second)
        W_avg[i, j] = float(np.mean(mon_w_RS.w[:, w_win] / b2.amp))
...
print(in_inh, in_exc, FREQ_avg.flatten(), FREQ_std.flatten(), W_avg.flatten())

print('... Saving .dat files...')
data = np.column_stack((in_inh, in_exc, FREQ_avg.flatten(),
                        FREQ_std.flatten(), W_avg.flatten()))
header = 'input_inh input_exc avg_f_out std_f_out avg_w'
# Per-column format: the previous global '%.2f' would round the adaptation
# current (~1e-10 A) to 0.00 and destroy the column. '%.6g' preserves it.
fmt = ['%.2f', '%.2f', '%.2f', '%.2f', '%.6g']
np.savetxt(os.path.join(simualtion_folder,'testing_TF_data_RS_w.dat'),
           data, fmt=fmt, header=header, comments='')
