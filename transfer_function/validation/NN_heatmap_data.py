...
from TF_helper import generate_ou_process
...
p_start, p_end = 1 * b2.second, 6 * b2.second
...
for exc_val in exc_freq_range:
    for inh_val in inh_freq_range:
        ...
        rate_timed_array_exc_all = b2.TimedArray(generate_ou_process(times/b2.second, dt/b2.second, 0.1, tau_ou, sigma_ou)*b2.Hz, dt=dt)
        ...
        P_ed_exc_all = b2.PoissonGroup(N_ext_exc, rates='rate_timed_array_exc_all(t)')
        ...
        S_ed_background_FS.connect(p=ext_conn_prob)
        S_ed_background_RS = b2.Synapses(P_ed_exc_all, G_RS, on_pre='GsynE_post+=Qe_RS')
        S_ed_background_RS.connect(p=ext_conn_prob)
        ...
        b2.run(sim_duration, namespace={'current': current_timed_array,
                                'rate_timed_array_exc_all': rate_timed_array_exc_all,
        ...
        sim_path = os.path.join('simulations/val_NN_[0_30]')
        ...
        mean_rates, std_rates = extracting_pop_freq_and_std(sim_duration=sim_duration, p_start=p_start, p_end=p_end,
